...
void loop() 
{
    ...
    if (millis() > next_publ)
    {
        if ( bme.performReading()) 
        {
            ...
            ctrl.temp = bme.humidity;
            switch(sindx)
            {
              case 0:
                Serial.print("Temperature = "); Serial.print(ctrl.temp);
                ...
                break;
              case 1:
                Serial.print("Humidity = "); Serial.print(ctrl.hum);
                if (! sensor_humidity.publish(ctrl.hum))  //Publish to Adafruit                
                    Serial.println(F(" - Failed"));
                else 
                    Serial.println(F(" - Sent!"));
                sindx = 0;
